...
def computeSimilarityMetric(video_volume):
 ...
 #           sim_matrix[i, i+j+1] = rssd
 #           sim_matrix[i+j+1, i] = rssd
 #
 #   return sim_matrix / np.average(sim_matrix)
    flatten_imgs = video_volume.reshape(num_img,-1).astype(np.float)
    
    for i in range(num_img):
        start_frame = flatten_imgs[i]
        for j in range(i+1,num_img):
            end_frame = flatten_imgs[j]
...
def transitionDifference(similarity):
    ...
    diff_matrix = cv2.filter2D(similarity, -1, kernel)
    
    return diff_matrix[2:-2, 2:-2]
    ...
    **************************************************************************
    ...
    for i_start in range(len(transition_diff)):
        for i_end in range(i_start+1, len(transition_diff[0])):
            score = alpha * (i_end - i_start) - transition_diff[i_start,i_end]
            if score > max_score:
                max_score = score
                start = i_start
                end = i_end
